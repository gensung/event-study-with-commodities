...
rdp.open_desktop_session('8a6bf978f1d54deaaa5f395066bd3d327c3f27cb')
...
event_date = pd.read_csv('/Users/angel/Desktop/zhaw/2nd Sem/AQM/Proj_Tomasz/data/FOMC dates-github.csv')
...
#No 2 Event study using 20 days event window and estimation window of 120 days
def compute_market_model_parameters(df, ticker, market_ticker):
    # Perform linear regression
    beta, alpha = np.polyfit(df[market_ticker], df[ticker], 1)
    return alpha, beta
...
event_window = list(range(-20, 21))  # From 20 days before the event to 20 days after
...
for day in event_window:
    ...
    for index, row in event_dates.iterrows():
        ...
        # Define the start and end dates of the estimation window (e.g., 120 days before the event)
        est_start_date = event_date - pd.DateOffset(days=120)
        est_end_date = event_date - pd.DateOffset(days=1)
        ...
        # Compute the abnormal return for the current day
        if not current_day_returns.empty:
            # Compute the expected return based on the market model
            expected_return = alpha + beta * current_day_returns[market_ticker]
            actual_return = current_day_returns[ticker]
...
#Calculating 95% Confidence interval for AAR and CAAR
...
for day in event_window:
    ...
    # Compute the confidence interval for AAR and its standard error
    aar_lower_bound, aar_upper_bound, std_err_aar = calculate_confidence_interval(abnormal_returns, confidence=0.95)

    # Compute the confidence interval for CAAR and its standard error
    caar_lower_bound, caar_upper_bound, std_err_caar = calculate_confidence_interval(results['caar'] + [caar], confidence=0.95)
...
#No 3 Event study process using 30 days event window, 250 estimation window
def compute_market_model_parameters(df, ticker, market_ticker):
    # Perform linear regression
    beta, alpha = np.polyfit(df[market_ticker], df[ticker], 1)
    return alpha, beta
...
def perform_t_test(abnormal_returns):
    # Perform a t-test for the null hypothesis that the mean is zero
    t_stat, p_value = stats.ttest_1samp(abnormal_returns, 0)
    return t_stat[0], p_value[0]

# Define the event window
event_window = list(range(-30, 31))  # From 30 days before the event to 30 days after
...
for day in event_window:
    ...
    for index, row in event_dates.iterrows():
        ...
        # Define the start and end dates of the estimation window (e.g., 250 days before the event)
        est_start_date = event_date - pd.DateOffset(days=250)
        est_end_date = event_date - pd.DateOffset(days=1)
...
#Calculating 99% confidence interval for AAR and CAAR 
def calculate_confidence_interval(data, confidence=0.99):
    """Calculate the confidence interval for a given dataset."""
    mean = np.mean(data)
    standard_error = np.std(data) / np.sqrt(len(data))
    z_value = 2.58  # Z-value for a 99% confidence interval
...
for day in event_window:
    ...
    # Compute the confidence interval for AAR and its standard error
    aar_lower_bound, aar_upper_bound, std_err_aar = calculate_confidence_interval(abnormal_returns, confidence=0.99)

    # Compute the confidence interval for CAAR and its standard error
    caar_lower_bound, caar_upper_bound, std_err_caar = calculate_confidence_interval(results['caar'] + [caar], confidence=0.99)
