...
# Initialize a DataFrame to store the abnormal returns for each day and each event
abnormal_returns = pd.DataFrame(index=event_dates.index, columns=event_window)
...
for day in event_window:
    ...
    for index, row in event_dates.iterrows():
        ...
        if not current_day_returns.empty:
            ...
            expected_return = alpha + beta * current_day_returns[market_ticker].values[0]
            actual_return = current_day_returns[ticker].values[0]
            abnormal_return = actual_return - expected_return
            abnormal_returns.loc[index, day] = abnormal_return

    # Computing the AAR for the current day
    aar = abnormal_returns[day].mean()

    # Computing the CAAR up to the current day
    caar = np.sum(results['aar'] + [aar])

    # Performing the t-test
    t_stat, p_value = perform_t_test(abnormal_returns[day].dropna())
...
#Calculating 90% confidence interval for AAR and CAAR 

def calculate_confidence_interval(data, confidence):
    # Compute the mean and standard error of the data
    mean = np.mean(data)
    std_err = stats.sem(data)
    # Compute the bounds of the confidence interval
    lower_bound = mean - std_err * stats.t.ppf((1 + confidence) / 2., len(data)-1)
    upper_bound = mean + std_err * stats.t.ppf((1 + confidence) / 2., len(data)-1)
    return lower_bound, upper_bound, std_err

results = {'day': [], 'aar': [], 'aar_lower': [], 'aar_upper': [], 'std_err_aar': [], 'caar': [], 'caar_lower': [], 'caar_upper': [], 'std_err_caar': [], 't_stat': [], 'p_value': []}

# Iterate over each day in the event window
for day in event_window:
    # Initialize cumulative abnormal returns
    ...
    for index, row in event_dates.iterrows():
        ...
        if not current_day_returns.empty:
            ...
            abnormal_return = actual_return - expected_return
            abnormal_returns.loc[index, day] = abnormal_return
            cumulative_abnormal_returns.append(abnormal_return)

    # Compute the AAR for the current day
    aar = np.mean(abnormal_returns.iloc[:, day])

    # Compute the CAAR up to the current day
    caar = np.sum(cumulative_abnormal_returns)

    # Compute the confidence interval for AAR and its standard error
    aar_data = abnormal_returns.iloc[:, day].dropna()
    aar_lower_bound, aar_upper_bound, std_err_aar = calculate_confidence_interval(aar_data, confidence=0.90)

    # Compute the confidence interval for CAAR and its standard error
    caar_lower_bound, caar_upper_bound, std_err_caar = calculate_confidence_interval(cumulative_abnormal_returns, confidence=0.90)

    # Compute t-stat and p-value
    t_stat, p_value = stats.ttest_1samp(aar_data, 0)
...
## if doing multiple events, apply both event study and modify it according to the event window, estimation window and confidence interval
